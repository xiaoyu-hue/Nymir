...
- **入口**：`npm test`（Vitest），287 项，31 个测试文件（`src/__tests__/`），覆盖：crypto/e2ee/fingerprint/keyRotation(+Verifiable)/burn/db/backup/base64/id/噪声、UI 组件（BurnTimer/ConfirmDialog/ErrorBoundary/GlassCard）等。
