...
在一个数据被无处不在地收集、监控、分析的时代，**隐私**已经成为一种奢侈品。

Nymir 诞生于一个简单的信念：**每个人都应该拥有秘密的权利。**

它是一款**完全去中心化**的匿名聊天应用。没有后端服务器，没有数据库，没有数据收集，没有"我们可能会分享你的数据"的条款。所有通信通过 **WebRTC 端到端加密** 直接在用户设备之间进行，所有数据存储在你自己的浏览器中。

消息可以设置为**阅读即焚** — 被查看后自动销毁，不留痕迹。就像从未存在过一样。
...
- **零注册** — 无需手机号、邮箱、任何个人信息
- **随机身份** — 每次连接自动生成匿名 ID
- **端到端加密** — WebRTC 直连，消息内容不可被第三方窃听
- **零数据收集** — 无埋点、无追踪、无日志、无分析
...
| **永不销毁** | 永久保留在本地 | 重要记录、长期保存 |

</div>

### 🌐 去中心化 P2P

Nymir 的通信架构基于**三层降级策略**，确保最大可用性：
...
视觉设计灵感来自 iOS 的玻璃拟态风格：

- **Glassmorphism** — 半透明毛玻璃效果，三层变体（默认/强/柔）
- **星空背景** — 120 颗闪烁星星 + 随机流星动画
- **微动态交互** — 消息销毁动画、重连脉冲指示器
- **移动端适配** — safe-area 适配、44px 触控区域、禁缩放
...
<br>
不需要。Nymir 完全无需注册，打开即可使用。每次连接自动生成随机匿名 ID。
</details>

<details>
<summary><b>Q: 消息会存储在服务器上吗？</b></summary>
<br>
不会。Nymir 没有服务器，所有消息通过 WebRTC P2P 直连传输，仅存储在你的浏览器本地。
</details>

<details>
<summary><b>Q: 阅后即焚真的安全吗？</b></summary>
<br>
阅后即焚消息在对方查看后会从双方设备的内存和存储中删除。但请注意，对方在查看的瞬间可能截屏，且浏览器内存中可能短暂存在明文。
</details>

<details>
<summary><b>Q: 不同网络的设备能互相连接吗？</b></summary>
<br>
可以。通过 BitTorrent Tracker 和 MQTT Broker 进行信令交换，不同网络的设备也能建立 P2P 连接。同一局域网内连接更快。
</details>
