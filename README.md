...
**[🔗 在线体验（GitHub Pages 主站）](https://xiaoyu-hue.github.io/Nymir/) · [备用副站（Cloudflare Pages）](https://nymir.pages.dev/)**
...
- **信令/建连阶段不是端到端加密通道**
- **无服务端离线收件箱**
- **自动密钥轮换策略未启用** — 支持**可验证密钥轮换**（签名衔接协议：换钥用旧签名私钥签名新公钥，对端验签通过才 re-pin，见 [ADR-007](./docs/adr/0007-verifiable-key-rotation.md)）；显式 API 与 UI 入口已就绪（手动轮换，v1.3.0），自动轮换策略待后续
- **假名与设备绑定有限**
- **双端实时依赖浏览器与 NAT**
...
- [x] 可验证密钥轮换协议（签名衔接，显式 API，ADR-007）
- [x] 密钥轮换 UI 入口（手动轮换 + 确认交互 + 结果反馈，v1.3.0）
- [x] CI 测试门禁（typecheck + lint + test）

### 计划中 🚀

- [ ] 密钥自动轮换策略（定时/条件触发，当前仅手动）
- [ ] 消息搜索、语音、多设备同步
- [ ] 降低对公共信令的依赖
